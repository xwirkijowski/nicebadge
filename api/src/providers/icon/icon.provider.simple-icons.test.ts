...
import SimpleIconsIconProvider from "@/providers/icon/icon.provider.simple-icons";
import {Icon} from "@/providers/icon/icon";

describe("SimpleIconsIconProvider Tests", () => {
    const provider = new SimpleIconsIconProvider();

    test("Can resolve icons from slugs by pulling from external API", async () => {
    ...
    describe("Rejects when expected", () => {
        test("No index 1 after encoded data split", async () => {
